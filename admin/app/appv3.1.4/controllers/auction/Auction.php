...
class Auction extends MY_Controller
{
    ...
    function add_submit()
    {
        ...
        foreach (array_pop($list_img_tmp) as $img_tmp) {
            $copy = copy_image_to_public_upload($img_tmp, FOLDER_AUCTION);

            $img_link = $copy['status'] ? ROOT_DOMAIN . FOLDER_AUCTION . $copy['basename'] : '';
    ...
    function edit_submit($id_article)
    {
        ...
        foreach ($list_img_remove as $img_removed) {
            ...
            @unlink($_SERVER["DOCUMENT_ROOT"] . '/' . FOLDER_AUCTION . $basename);
        }

        # chuyển ảnh mới từ TMP sang FOLDER_AUCTION
